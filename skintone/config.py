...
#:
#: ``screen`` = 自拍时的"屏幕光 + 室内灯"。**不要把它当成冷白屏**：
#: 屏幕此时显示的是摄像头画面（一张脸）和这个网页本身（暖纸色 #f4f1ea），
#: 也就是屏幕投出来的光是被暖色内容调制过的，不是 6500K 冷白。
#: 与一盏普通室内灯混合后，落在 4000K 附近——而不是我上一版拍脑袋写的 4800K。
#: 它是**前端**的默认项；API 的默认仍是 ``unknown``：后端不该替调用方假定"这是自拍"。
